...
Before asking, Hunk verifies that the path it would write is the file the prompt names. A reviewed path that is a symlink, or that sits under a directory link leading out of the repository, resolves `unavailable` rather than being followed, and so does a file that has left the working tree since the review was built — a write replaces a reviewed file, it never recreates a deleted one. `canWriteDocument` skips the filesystem and is optimistic about all three; the write itself is not.
